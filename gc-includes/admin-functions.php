...
*  $LastChangedDate: 2007-04-10 13:28:52 +0800 (ÊòüÊúü‰∫å, 10 ÂõõÊúà 2007) $
...
function saveEditUser($user_id,$user_pass)
{
	...
	if($user_pass!="")
	{
    	...
    	$objResponse->addAssign("lo","innerHTML",'√‹¬Î≤ªƒ‹Œ™ø’');
...
function saveAddUser($user_name,$user_pass)
{
	...
	if ($user_name=="")
	{
    	$objResponse->addAssign("lo","innerHTML",'”√ªß√˚≤ªƒ‹Œ™ø’');
    	$objResponse->addAssign("lo","style.background",'#c44');  
	}
	elseif($user_pass=="")
	{
...
function saveEditLink($link_ID,$link_name,$link_url){
	...
	{    
	   if(get_option('charset')=='gb2312'){
    	...
    	$objResponse->addAssign("lo","innerHTML",'√˚≥∆∫ÕURL≤ªƒ‹Œ™ø’');
    	$objResponse->addAssign("lo","style.background",'#c44');  
	}
...
function saveDeleteLink($link_ID){
	...
	{    
    	...
    	$objResponse->addAssign("lo","innerHTML",'Œﬁ∑®…æ≥˝∏√¡¥Ω”');
    	$objResponse->addAssign("lo","style.background",'#c44');
	}
...
function saveAddLink($link_name,$link_url)
{
	...
	if ($link_name=="")
	{
    	$objResponse->addAssign("lo","innerHTML",'¡¥Ω”√˚≥∆≤ªƒ‹Œ™ø’');
    	$objResponse->addAssign("lo","style.background",'#c44');  
	}
	elseif($link_url=="")
	{
    	$objResponse->addAssign("lo","innerHTML",'URL≤ªƒ‹Œ™ø’');
    	$objResponse->addAssign("lo","style.background",'#c44');  
	}
	else {
	   if(get_option('charset')=='gb2312'){
...
function saveEditTag($tag_ID,$tag_name,$tag_description,$tag_parent){
	...
	{    
    	...
    	$objResponse->addAssign("lo","innerHTML",'TAG√˚≥∆≤ªƒ‹Œ™ø’');
    	$objResponse->addAssign("lo","style.background",'#c44');  
	}
...
function saveNoShowTag($tag_ID){
	...
	{    
    	...
    	$objResponse->addAssign("lo","innerHTML",'Œﬁ∑®“˛≤ÿ∏√TAG');
    	$objResponse->addAssign("lo","style.background",'#c44');
	}
...
function saveEditSpam($spam_ID,$spam_value,$spam_type){
	...
	if($spam_value!=""&&($spam_type=="name"||$spam_type=="email"||$spam_type=="text"||$spam_type=="ip"))
	{
	   if(get_option('charset')=='gb2312'){
    	...
    	$objResponse->addAssign("lo","innerHTML",' ‰»Î¥ÌŒÛ');
    	$objResponse->addAssign("lo","style.background",'#c44');  
	}
...
function saveDeleteSpam($spam_ID){
	...
	{    
    	...
    	$objResponse->addAssign("lo","innerHTML",'Œﬁ∑®…æ≥˝');
    	$objResponse->addAssign("lo","style.background",'#c44');
	}
...
function saveAddSpam($spam_value,$spam_type)
{
	...
	if ($spam_value=="")
	{
    	$objResponse->addAssign("lo","innerHTML",'÷µ≤ªƒ‹Œ™ø’');
    	$objResponse->addAssign("lo","style.background",'#c44');  
	}
	elseif($spam_type!="name"&&$spam_type!="email"&&$spam_type!="text"&&$spam_type!="ip")
	{
    	$objResponse->addAssign("lo","innerHTML",'¿‡–Õ¥ÌŒÛ');
    	$objResponse->addAssign("lo","style.background",'#c44');  
	}
	else {
	   if(get_option('charset')=='gb2312'){
...
function saveAddX($post_ID,$x_name)
{
	...
	{
    	$objResponse->addAssign("lo","innerHTML",'÷µ≤ªƒ‹Œ™ø’«“±ÿ–Ë « ˝◊÷');
    	$objResponse->addAssign("lo","style.background",'#c44');  
	}
	...
	{
    	$objResponse->addAssign("lo","innerHTML",'πÿº¸◊÷¥ÌŒÛ');
    	$objResponse->addAssign("lo","style.background",'#c44');
	}
	else {
	   if(get_option('charset')=='gb2312'){
